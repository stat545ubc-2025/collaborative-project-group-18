...
``` r
#fixed errors by changing load.packages to library
```
...
``` r
### ERROR HERE ###
class(dslabs::movielens)#is a data.frame
```

    ## [1] "data.frame"

``` r
movieLens <- as_tibble(dslabs::movielens) #convert to tibble and stored in movieLens
class(movieLens)#checking class of movieLens
```

    ## [1] "tbl_df"     "tbl"        "data.frame"

``` r
dim(movieLens)#gives rows and columns of movieLens
```

    ## [1] 100004      7

``` r
glimpse(movieLens)#added this for a quick look at data
```
...
``` r
filter(movieLens, year > 2000) #changed to be filmed after 2000, not including 2000
```
...
``` r
filter(movieLens, year == 1999 | year == 2000)#changed from month == 2000 to year == 2000
```
...
movieLens %>%
  filter(!genres == "Drama") %>%
  select(title, genres, everything(movieLens))#added function everything() to select everything else
```
